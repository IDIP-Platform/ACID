...
def process_image_mask(image, mask, input_channel_axis=-1):
    ...
    data = (
        image.copy()
    )  # this is a placeholder for any processing that needs to be done
    mask = (
        image.copy()
    )  # this is a placeholder for any processing that needs to be done
...
def f_mbfm(filenames, chunks=(256, 256, -1), kwargs=None):
    ...
    for filename in filenames:
        ...
        fm = compute_feature_map(data, chunks=chunks, **kwargs)
