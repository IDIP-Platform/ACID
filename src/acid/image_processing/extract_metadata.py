...
def extract_bioio_scene_metadata(
    bioio_scene: Callable,
    ...
    **kwargs,
    ...
    assert (
        isinstance(dtype_name, str) or dtype_name == False
    ), "dtype_name can only be a string or False"
    assert (
        isinstance(dims_order_name, str) or dims_order_name == False
    ), "dims_order_name can only be a string or False"
    assert (
        isinstance(t_name, str) or t_name == False
    ), "t_name can only be a string or False"
    assert (
        isinstance(c_name, str) or c_name == False
    ), "c_name can only be a string or False"
    assert (
        isinstance(z_name, str) or z_name == False
    ), "z_name can only be a string or False"
    assert (
        isinstance(xy_prefix_name, str) or xy_prefix_name == False
    ), "xy_root_name can only be a string or False"
    assert (
        isinstance(physical_size_prefix_name, str) or physical_size_prefix_name == False
    ), "physical_size_suffix_name can only be a string or False"
        ...
        # add the number of planes to the metadata dictionary and if z_name is different than False
            ...
            if z_name != False:
                ...
                if dim_size > 1:
                    ...
                    scene_metadata[
                        f"{physical_size_prefix_name}_{dim_name.lower()}"
                    ] = input_scene_dim_physical_size
