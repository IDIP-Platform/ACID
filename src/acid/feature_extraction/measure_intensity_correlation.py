...
def measure_corr_coeff_per_label_single_pair(
    ...
    **kwargs,
    ...
    ), "method can't be passed to kwargs. Use the method argument instead."

    if return_permutations:
        assert (
            n_permutation > 0
        ), "return_permutations can only be True if n_permutation > 0"

    # split the intensity_image_stack along the specified axis
    intensity_image_list = [
        np.squeeze(a, axis=axis)
        for a in np.split(
            intensity_image_stack,
            indices_or_sections=intensity_image_stack.shape[axis],
            axis=axis,
        )
    ]
    ...
    # If permutations are requested, perform them
        ...
        assert (
            "replace" not in kwargs
        ), "replace can't be passed to kwargs and it is by default set to False in permute_values_per_label function"
        assert (
            ignore_index not in kwargs
        ), "ignore_index can't be passed to kwargs. Use the ignore_index argument instead."
        ...
        for i in range(n_permutation):
            ...
            permutation_df = permute_values_per_label(
            ...
            # Group by label, apply correlation, unstack the result and select the relevant correlation coefficients
            i_correlation_df = (
                permutation_df.groupby("label")[["value_0", "value_1"]]
                .corr(method=method, min_periods=min_periods, numeric_only=numeric_only)
                .unstack()
                .iloc[:, 1]
            )
        ...
        collection_permutation_df = pd.concat(
            collection_permutation, axis=1, ignore_index=ignore_index
        )

        # Calculate the mean and standard deviation of the correlation coefficients for each label
        collection_permutation_df[f"permuted_mean_{method}_coeff"] = (
            collection_permutation_df.mean(axis=1)
        )
        collection_permutation_df[f"permuted_std_{method}_coeff"] = (
            collection_permutation_df.std(axis=1)
        )

        # Join the observed correlation coefficients with the permuted ones
        correlation_df = collection_permutation_df[
            [f"permuted_mean_{method}_coeff", f"permuted_std_{method}_coeff"]
        ].join(correlation_series.rename(f"{method}_coeff"), how="right")
...
def measure_corr_coeff_per_label(
    label_image: np.array,
    intensity_image_stack: np.array,
    axis: int = 0,
    n_permutation: int = 0,
    ...
    **kwargs,
    ...
    for c_i, i in enumerate(intensity_image_list):
        ...
        for j in intensity_image_list[c_j:]:
            ...
            i_correlation_df = measure_corr_coeff_per_label_single_pair(
                intensity_image_stack=image_pair,
                label_image=label_image,
            ...
            if return_permutations:
                ...
                i_correlation_df[0].columns = [
                    f"{col}{sep}{c_i}_{c_j}" for col in i_correlation_df[0].columns
                ]
                i_correlation_df[1].columns = [
                    f"{col}{sep}{c_i}_{c_j}" for col in i_correlation_df[1].columns
                ]
            ...
            # if return_permutations is False, collect only the correlation coefficients for the current pair of images to the collection
                ...
                if n_permutation > 0:
                    i_correlation_df.columns = [
                        f"{col}{sep}{c_i}_{c_j}" for col in i_correlation_df.columns
                    ]
                else:
                    i_correlation_df.rename(
                        f"{method}_coeff{sep}{c_i}_{c_j}", inplace=True
                    )
