...
from acid.image_measurement.measure_local_image_stat import (
    aggregate_local_stat_along_axis,
)
...
def measure_local_mean_over_std(
    ...
    if local_kwargs == None:
        ...
        if "axis" in local_kwargs:
            assert (
                local_kwargs["axis"] == None
            ), "if axis is passed to local kwargs, it must be None"
        else:
            local_kwargs["axis"] = None

    return aggregate_local_stat_along_axis(
        image,
