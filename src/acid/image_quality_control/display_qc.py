...
def high_low_QC_fov(
    metadata_df, qc_clm, fov_clm, ch_measurement_sep, fov_dir, channel_ax
):
    ...
    img_low = img_low.take(
        indices=int(qc_clm.split(sep=ch_measurement_sep)[-1]), axis=channel_ax
    )
    img_high = img_high.take(
        indices=int(qc_clm.split(sep=ch_measurement_sep)[-1]), axis=channel_ax
    )
...
def display_high_low_qc(
    ...
    try:
        for col in qc_clm_collection:
            ...
            img_low, img_high, fov_row_low, fov_row_high = high_low_QC_fov(
                metadata_df,
                qc_clm=col,
                fov_clm=fov_clm,
...
def display_qc(
    ...
    for row in df_to_display.index:
        ...
        # open the images
        fov_to_display = tifffile.imread(
            os.path.join(fov_directory, str(fov_to_display_name))
        )

        # select the channel to visualize
        fov_ch_to_display = fov_to_display.take(
            indices=channel_to_display, axis=channel_axis
        )

        # visualize using napari
        napari_viewer.add_image(
            fov_ch_to_display,
            name=f"{qc_to_display}: {metadata_df.loc[row, f"{qc_to_display}{ch_measurement_separator}{channel_to_display}"]}, {fov_to_display_name}",
        )


def plot_qc_by_channel_condition(
    metadata_df,
    qc_clm,
    condition_clm,
    ax,
    ...
    assert (
        "ax" not in violin_kws and "ax" not in strip_kws
